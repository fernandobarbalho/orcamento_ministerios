...
dados_orcamento_2023%>%
  mutate(pago_empenhado = (pago/empenhado)*100) %>%
  ggplot(aes(x= pago/10^9, y= empenhado/10^9)) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +
  geom_point( aes(fill= pago_empenhado, color= pago_empenhado ),   pch= 21, show.legend = TRUE)+
  geom_text_repel(data= destaque, 
                  aes(x= pago/10^9, 
                      y= empenhado/10^9, 
                      label= str_wrap(  paste(nome_curto, "(E:",round(empenhado/10^9,1) , "P:", round(pago/10^9,1),")"),60)),  
                  box.padding = 0.8,
                  nudge_y = -0.5,
                  size= 2.5,
                  hjust = -0.1) +
