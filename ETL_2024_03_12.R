library(tidyverse)
library(readxl)
dados_orcamento_trabalho<- read_delim("dados_orcamento_2024.csv", 
                                   delim = ";", escape_double = FALSE, locale = locale(decimal_mark = ",", 
                                                                                       grouping_mark = "."), trim_ws = TRUE)
dados_orcamento_trabalho<- dados_orcamento_trabalho[-1,]

dados_orcamento_trabalho<- janitor::clean_names(dados_orcamento_trabalho)
...
dados_orcamento_trabalho %>%
  filter(orgao_orcamentario %in% ministerios) %>%
  saveRDS("dados_orcamento_trabalho.rds")
...
dados_orcamento_trabalho %>%
  filter(orgao_orcamentario %in% ministerios) %>%
  summarise(
    dotacao_inicial = sum(dotacao_inicial),
...
dados_orcamento_trabalho_ministerios$nome_curto<- nomes_sugeridos

dados_orcamento_trabalho_ministerios%>%saveRDS("dados_orcamento_trabalho_ministerios.rds")
